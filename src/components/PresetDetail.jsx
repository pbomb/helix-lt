...
function BlockCard({ block, snapshotNames }) {
...
export default function PresetDetail({ preset, onBack }) {
  const [playingNotesOpen, setPlayingNotesOpen] = useState(false)

  const snapshotNames = Object.keys(preset.snapshots)

  const orderedBlocks = preset.signal_chain.map(modelName => {
    const entry = Object.entries(preset.blocks).find(([, b]) => b.model === modelName)
    if (!entry) return null
    return { key: entry[0], ...entry[1] }
  }).filter(Boolean)

  const { tuning, key: key_, capo, technique, snapshot_cues, tone_control, pickup_notes, reference } = preset.playing_notes || {}
    ...
    <div className="preset-detail">
      ...
      <header className="detail-header">
        ...
        <div className="detail-meta">
          ...
          {preset.playing_notes?.capo && <><span className="meta-sep">·</span><span>Capo {preset.playing_notes.capo}</span></>}
        </div>
